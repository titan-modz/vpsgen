...
TOKEN = 'MTM5NjQwMDA0NDIzNTU1ODkyMg.G5jH0l.gVD5NTrOBz2PTsaLXoFjI6a7EkGZ3xNhi8Tigk' # TOKEN HERE
RAM_LIMIT = '8g'
SERVER_LIMIT = 1
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="Creating Instance with ZYRIXVPSGEN", color=0x00ff00))
    user = str(interaction.user)
    ...
    if ssh_session_line:
        ...
        await interaction.followup.send(embed=discord.Embed(description="Instance created successfully with ZYRIXVPSGEN. Check your DMs for details.", color=0x00ff00))
    else:
...
@bot.tree.command(name="deploy-ubuntu", description="Creates a new Instance with ZYRIXVPSGEN")
async def deploy_ubuntu(interaction: discord.Interaction):
    await create_server_task(interaction)

@bot.tree.command(name="deploy-debian", description="Creates a new Instance with ZYRIXVPSGEN")
async def deploy_ubuntu(interaction: discord.Interaction):
...
async def list_servers(interaction: discord.Interaction):
    ...
    if servers:
        ...
        for server in servers:
            ...
            embed.add_field(name=container_name, value="Description: A VPS BY ZYRIXVPSGEN WITH AMD Ryzen 9 9950X3D.", inline=False)
        await interaction.response.send_message(embed=embed)
    else:
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(name="/deploy-ubuntu", value="Creates a new Instance with ZYRIXVPSGEN.", inline=False)
    embed.add_field(name="/deploy-debian", value="Creates a new Instance with ZYRIXVPSGEN.", inline=False)
